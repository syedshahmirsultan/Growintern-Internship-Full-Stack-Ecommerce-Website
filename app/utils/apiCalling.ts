...
export async function getAllCartProductsByUserid(userid:string){
  const res = await fetch(`https://shahmir-ecommerce-website.vercel.app/api/cartFunc?userid=${userid}`,{
    cache : "no-store"
  })
  ...
  export async function addToCartApiCall(userid: string, productid: string) {
    ...
    const res = await fetch(`https://shahmir-ecommerce-website.vercel.app/api/cartFunc`, {
        method: "POST",
        headers: {
...
export async function updateCartItem(userid:string,productid:string,quantity:number){
 ...
 export async function handleDelete(userid:string,productid:string){
    const res = await fetch(`https://shahmir-ecommerce-website.vercel.app/api/cartFunc?userid=${userid}&productid=${productid}`,{
 method :"DELETE"
    })
    await refreshData();
